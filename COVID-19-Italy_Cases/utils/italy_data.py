...
def parse_provinces() -> list:
    ...
    #provinces = list(set([element.get("denominazione_regione") for element in data]))

    #default_provinces = ["VARESE", "BERGAMO", "MILANO", "COMO", "ROME"]
...
def compute_provinces_confirmed_cases() -> dict:
    ...
    for p in all_provinces:
        ...
        #local_data = [element.get("totale_casi") for element in data if element.get("denominazione_regione") == p]
        new_local_data = list()
